<template>
  <div id="app" class="main">
    ...
    <div class="details">
      <div class="item">
        <div>
          {{ details.lineName }}-{{ details.machineName }}-{{ details.doffNo }}
        </div>
    ...
    <div class="car-list">
      <div
        class="car-item"
        v-for="(item, index) in details.silkReportVoList"
        :key="index"
      >
        <div>锭位数:{{ item.spindleNum }}</div>
        <div>丝锭号:{{ item.silkCode }}</div>
        <div>等级:{{ item.grade }}</div>
        <div v-if="item.packageBoxCode != null">
          位置:{{ item.packageBoxCode }}
        </div>
        <div v-else>
