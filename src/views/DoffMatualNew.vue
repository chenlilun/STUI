    ...
    export default {
        ...
        methods: {
            ...
            jumpJieBang() {
                ...
                this.$router.push({
                    path: '/DoffAuto',
                    name: 'DoffAuto', //要跳转的路径的 name,在 router 文件夹下的 index.js 文件内找
                    /*params: {
                        key: 'silkCodeJump',
            ...
            allCarJieBang(){
                ...
                this.$api.silkUnbind({
                    ...
                    silkCarRowColList: this.silkCarRowColList,
                    separateFlag:true
                }).then((res) => {
                    if (res.data.status === '200') {
                        this.silkCodeList = []
            ...
            dingDeng() {
                Dialog.confirm({
                    title: '整车解绑',
                    message: '丝车资料与实物不符合,确认整车解绑？',
                })
                    .then(() => {
                        // on confirm
                        if(this.silkCarCode&&this.silkCarRowColList&&this.silkCarRowColList.length>0 ){
                            this.allCarJieBang()
                        }else {
                            Toast('没扫丝车，或者该车是空车可以落筒')
                        }
