<template>
  <div id="app">
    ...
    <van-cell-group>
      <van-cell
        title="报表日期"
        :value="this.data.length > 0 ? getTime(data[0].createTime) : ''"
      />
    </van-cell-group>
    <div style="display: flex">
      <van-button
        type="primary"
        style="justify-content: space-between; flex-wrap: wrap; width: 49%"
        @click="chooseTime(true)"
        >开始时间:{{ getTime2(startDate) }}</van-button
      >
      <van-button
        type="primary"
        style="justify-content: left; width: 49%; margin-left: 5px"
        @click="chooseTime(false)"
        >结束时间:{{ getTime2(endDate) }}</van-button
      >
    </div>

    <div class="divItem" v-for="(item, index) in data" :key="index">
      <div class="onerow">
        <div class="col_item">
          {{
            "台车:" +
            item.silkCarCode.substr(
              item.silkCarCode.length - 3,
              item.silkCarCode.length
            )
          }}
        </div>
      ...
      <div class="onerow">
        <div class="col_item_four">
          {{ "AA:" + getSilkCount("AA", item.spindleCount) }}
        </div>
        <div class="col_item_four">
          {{ "A：" + getSilkCount("A", item.spindleCount) }}
        </div>
        <div class="col_item_four">
          {{ "B:" + getSilkCount("B", item.spindleCount) }}
        </div>
        <div class="col_item_four">
          {{ "C:" + getSilkCount("C", item.spindleCount) }}
        </div>
      </div>
      <div class="onerow">
  ...
  methods: {
    ...
    getTime2: function (date) {
      ...
      return date === null || date === {}
        ? ""
        : moment(b).format("YYYY-MM-DD HH:mm:ss");
    },
