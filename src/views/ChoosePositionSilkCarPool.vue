            ...
            <li class="sudoku_item" :class="curSelect==silk.id?'opacity':''" v-for="(silk,index) in silks"
                ...
                <div :class="getColor(silk ,index)" @click="chooseSilk(silk,index)">
                    ...
                    <div style="color: white; border: 1px solid red; border-radius: 6px;background-color: red"
                         v-if="silk.silkCode!=''&&!silk.forbidChoose"
                         @click="delSilk(index)"
                    >
                        删除
                    </div>
        ...
        methods: {
            ...
            dingDeng() {
                ...
                this.silks.forEach(a=>{
                    if(a.silkCode){
                        if(a.forbidChoose!=null&&!a.forbidChoose){
                            arr.push(a)
                        }
                ...
                if(arr.length>0){
                    this.$api.ChoosePostionSilkCarPool({
                        operator: this.name,
                        id: this.data.id,
            ...
            onClickRight() {
                // this.callByAndroid('0J46539B00B12D')
                this.show = true
            },
            callByAndroid(code) {
                // Toast("对了？" + code)
                    ...
                    if (this.$myUtils.checkIsSilkCar(code)) { // 丝车
                        ...
                        if (this.silkCarCode) {
                            ...
                            if (this.isContentThisSilk(code, arr)) {
                                ...
                                this.silks[this.chooseIndex].forbidChoose = false
                                this.chooseIndex = -1
                            ...
                            for (let i = 0; i < arr.length; i++) {
                                for (let j = 0; j < this.silks.length; j++) {
                                    if((arr[i].sideType+arr[i].row+arr[i].col)===this.silks[j].position){
                                        ...
                                        this.silks[j].forbidChoose = true
                                    }
                                }
                            }
